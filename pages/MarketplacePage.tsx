  ...
  const filteredProducts = useMemo(() => {
    ...
    const getPrice = (p: Product) => {
      if (p.discount && p.discount.isActive) {
        return p.price * (1 - p.discount.percentage / 100);
      }
      return p.price;
    };

    if (sortBy === 'price-asc') result.sort((a, b) => getPrice(a) - getPrice(b));
    if (sortBy === 'price-desc') result.sort((a, b) => getPrice(b) - getPrice(a));
            ...
            <div className="flex flex-nowrap items-center gap-2 overflow-x-auto scrollbar-hide py-2 px-1">
              {categories.map(cat => (
                <button
                  ...
                  className={`whitespace-nowrap px-4 md:px-6 py-2.5 md:py-3.5 rounded-full font-black text-[10px] md:text-xs transition-all uppercase tracking-widest flex-shrink-0 border-2 ${filter === cat
