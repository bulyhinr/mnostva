      ...
      if (limit) {
        try {
          ...
          const mappedProducts: Product[] = (response.data || []).map((p: any) => ({
            ...
            features: Array.isArray(p.features) ? p.features : (typeof p.features === 'string' ? (p.features.startsWith('[') ? JSON.parse(p.features) : p.features.split(',').filter(Boolean)) : []),
            packContent: Array.isArray(p.packContent) ? p.packContent : (typeof p.packContent === 'string' ? (p.packContent.startsWith('[') ? JSON.parse(p.packContent) : p.packContent.split(',').filter(Boolean)) : []),
            compatibility: Array.isArray(p.compatibility) ? p.compatibility : (typeof p.compatibility === 'string' ? (p.compatibility.startsWith('[') ? JSON.parse(p.compatibility) : p.compatibility.split(',').filter(Boolean)) : []),
